...
func internalGetMatching(w http.ResponseWriter, r *http.Request) {
	...
	for _, ride := range rides {
		...
		for _, ch := range availableChairs {
			...
			dist := float64(manhattanDistance(ride.PickupLatitude, ride.PickupLongitude, location.LastLatitude, location.LastLongitude)*3+manhattanDistance(ride.PickupLatitude, ride.PickupLongitude, ride.DestinationLatitude, ride.DestinationLongitude)) / float64(chairModelSpeedCache[ch.Model])
			if dist < bestDist {
				bestDist = dist
			}
